...
result_path astar_algorithm::calculate(int dirs)
{
    directions = dirs;
    ...
    while (!open_set.empty())
    {
        ...
        // Taking advantage of branch predictor - this check will not happen, if mapview is
        // nullptr
        if (visualization) draw_change(*current);
        ...
        for (auto &neighbor : neighbors)
        {
            ...
            auto exists_in_set =
                std::find_if(open_set.begin(), open_set.end(),
                             [&](std::shared_ptr<node> const &p) { return p->pos == neighbor; });
    ...
    return prepare_result_path(start_node, current);
}
